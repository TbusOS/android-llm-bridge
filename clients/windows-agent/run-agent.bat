...
if errorlevel 1 (
    ...
    %PY% -c "import subprocess, csv; out = subprocess.run(['adb', 'devices'], capture_output=True, text=True, timeout=20).stdout; ds = [l.split('\t')[0] for l in out.splitlines()[1:] if '\t' in l]; print('  [v] adb ... OK, devices: ' + (', '.join(ds) if ds else 'NONE -- board not visible to adb yet')); rows = [] if ds else list(csv.reader(subprocess.run(['tasklist', '/fo', 'csv', '/nh'], capture_output=True, text=True, timeout=10).stdout.splitlines())); hits = [r[0] + ' pid=' + r[1] for r in rows if len(r) > 1 and 'adb' in r[0].lower().removesuffix('.exe') and r[0].lower().removesuffix('.exe') != 'adb']; hits and print('  [!] another adb build is running and likely holds the exclusive USB'); hits and print('  [!] interface: ' + ', '.join(hits)); hits and print('  [!] fix: taskkill /f /pid <pid>  then: adb kill-server + adb devices')" 2>nul
